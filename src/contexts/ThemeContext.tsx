...
export type ThemeMode = 'dark' | 'gray' | 'light' | 'nativecc' | 'custom';
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [theme, setThemeState] = useState<ThemeMode>('nativecc');
    ...
    const loadTheme = async () => {
      try {
        ...
        if (savedTheme) {
          ...
          // No saved preference: apply nativecc (Claude Desktop 风) as the default theme
          setThemeState('nativecc');
          await applyTheme('nativecc', customColors);
        }
  ...
  const applyTheme = useCallback(async (themeMode: ThemeMode, colors: CustomThemeColors) => {
    ...
    root.classList.remove('theme-dark', 'theme-gray', 'theme-light', 'theme-nativecc', 'theme-custom');
