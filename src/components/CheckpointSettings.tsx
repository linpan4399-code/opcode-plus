  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between pb-2 border-b border-border">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-caption text-muted-foreground mt-0.5">{t("checkpointSettings.description")}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-md border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/20 p-3">
        <div className="flex items-start gap-2.5">
          ...
          <div className="space-y-0.5">
            <p className="text-caption font-medium text-amber-900 dark:text-amber-100">{t("checkpointSettings.experimental.title")}</p>
            <p className="text-caption text-amber-700 dark:text-amber-300">
              {t("checkpointSettings.experimental.warning")}
            </p>
          </div>
        </div>
      </div>
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              <Label className="text-label">{t("checkpointSettings.storage.title")}</Label>
            </div>
            <p className="text-caption text-muted-foreground">
              {t("checkpointSettings.storage.totalCheckpoints")}: <span className="font-medium text-foreground">{totalCheckpoints}</span>
            </p>
          </div>
        </div>
        ...
        {/* Cleanup settings */}
        <div className="space-y-2">
          <Label htmlFor="keep-count" className="text-label">{t("checkpointSettings.storage.keepRecent")}</Label>
          <div className="flex gap-2">
            ...
            >
              ...
              >
                ...
                {t("checkpointSettings.storage.cleanupButton")}
              </Button>
            </motion.div>
          </div>
          <p className="text-caption text-muted-foreground">
            {t("checkpointSettings.storage.cleanupDescription", { count: keepCount })}
          </p>
        </div>
      </Card>
    </motion.div>
