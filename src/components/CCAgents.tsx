...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: t("toast.agentDeleted"), type: "success" });
      await loadAgents();
      await loadRuns(); // Reload runs as they might be affected
    } catch (err) {
      console.error("Failed to delete agent:", err);
      setToast({ message: t("toast.agentDeleteFailed"), type: "error" });
    } finally {
      setIsDeleting(false);
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("toast.agentCreated"), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("toast.agentUpdated"), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: t("toast.agentExported", { name: agent.name }), type: "success" });
    } catch (err) {
      console.error("Failed to export agent:", err);
      setToast({ message: t("toast.agentExportFailed"), type: "error" });
    }
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t("toast.agentImported"), type: "success" });
      await loadAgents();
    } catch (err) {
      console.error("Failed to import agent:", err);
      const errorMessage = err instanceof Error ? err.message : t("toast.agentImportFailed");
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
                    ...
                    {totalPages > 1 && (
                      <div className="mt-6 flex justify-center gap-2">
                        ...
                        <span className="flex items-center px-3 text-body-small">
                          {t("agents.page", { current: currentPage, total: totalPages })}
                        </span>
                        <Button
                          size="sm"
                          variant="outline"
      ...
      <GitHubAgentBrowser
        ...
        onImportSuccess={async () => {
          ...
          await loadAgents();
          setToast({ message: t("toast.agentImportedFromGitHub"), type: "success" });
        }}
      />
